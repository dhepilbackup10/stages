...
export class StagesLogicTransform {
  ...
  private hasTransformChanged(old: ViewportTransform, new_: ViewportTransform): boolean {
    ...
    return Math.abs(old.scale - new_.scale) > threshold ||
...
class TransformRules {
   ...
   */
  private calculateCoverTransform(viewportWidth: number, viewportHeight: number): ViewportTransform {
   ...
   */
  private calculateContainTransform(viewportWidth: number, viewportHeight: number): ViewportTransform {
    const scaleX = viewportWidth / this.stageWidth
    const scaleY = viewportHeight / this.stageHeight
