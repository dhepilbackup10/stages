...
class PerformanceRules {
   ...
   */
  private shouldReduceQuality(avgFPS: number, fpsHistory: number[], metrics: PerformanceMetrics): boolean {
    ...
    const highObjectCount = this.deviceTier ? metrics.objectCount > this.deviceTier.maxObjects * 0.8 : false
   ...
   */
  private shouldIncreaseQuality(avgFPS: number, fpsHistory: number[], metrics: PerformanceMetrics): boolean {
    ...
    const reasonableObjectCount = this.deviceTier ? metrics.objectCount < this.deviceTier.maxObjects * 0.6 : true
  ...
  private calculateQualityIncrease(currentQuality: RenderQuality): Partial<RenderQuality> {
    ...
    if (currentQuality.textureScale < deviceLimits.textureQuality) {
      adjustment.textureScale = Math.min(deviceLimits.textureQuality, currentQuality.textureScale * 1.1)
    }
