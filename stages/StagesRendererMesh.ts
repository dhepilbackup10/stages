...
export class StagesRendererMesh {
  ...
  private createLine(object: StageObject): THREE.Line {
    const { metadata } = object
    const points = metadata.points || [[0, 0], [100, 100]]
  ...
  private createClock(object: StageObject): THREE.Group {
    ...
    if (metadata.showHands) {
      ...
      const handMaterial = new THREE.MeshLambertMaterial({ color: metadata.handColor || 0x000000 })
   ...
   */
  private createDefaultMesh(object: StageObject): THREE.Mesh {
   ...
   */
  clearCache(): void {
    for (const [id, mesh] of this.meshCache) {
