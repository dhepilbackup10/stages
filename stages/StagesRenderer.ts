...
import { STAGE_WIDTH, STAGE_HEIGHT } from './StagesTransform'
...
export class StagesRenderer {
  ...
  constructor(performance: StagesPerformance) {
  ...
  start(): void {
    ...
    const render = () => {
      if (!this.paused && this.renderer && this.scene && this.camera) {
        this.performance.trackRenderCall()
  ...
  dispose(): void {
    ...
    for (const [id, mesh] of this.renderObjects) {
      this.meshFactory.disposeMesh(mesh)
    }
