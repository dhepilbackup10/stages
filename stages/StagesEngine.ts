...
import { StagesDevice } from './StagesDevice'
import { StagesTransform } from './StagesTransform'
import { StagesRenderer } from './StagesRenderer'
import { StagesPerformance } from './StagesPerformance'
...
export class StagesEngine {
  // Core modules (stable)
  private device: StagesDevice
  private transform: StagesTransform
  private renderer: StagesRenderer
  private performance: StagesPerformance
  ...
  constructor(config: StageConfig = {}) {
    ...
    this.device = new StagesDevice()
    this.transform = new StagesTransform()
    this.performance = new StagesPerformance()
    this.renderer = new StagesRenderer(this.performance)
    ...
    if (config.deviceTier && config.deviceTier !== 'auto') {
      this.device.setDeviceTier(config.deviceTier)
    }
  ...
  async mount(container: HTMLElement): Promise<void> {
    ...
    // Initialize renderer with device quality
    const quality = this.device.getRenderQuality()
    ...
    this.transform.initialize(container, canvas)

    // Setup event handlers (delegated to event manager)
    this.eventManager.setupEventHandlers(
      container,
      (event) => this.transform.transformEvent(event),
  ...
  setObject(id: string, objectData: Partial<StageObject>): void {
    ...
    this.performance.setObjectCount(this.objectManager.getCount())
  }
  ...
  transformCoordinates(event: PointerEvent | MouseEvent | TouchEvent): StageCoordinates | null {
    return this.transform.transformEvent(event)
  }
  ...
  transformPoint(clientX: number, clientY: number): StageCoordinates | null {
    return this.transform.transformCoordinates(clientX, clientY)
  }
  ...
  isWithinStage(stageX: number, stageY: number): boolean {
    return this.transform.isWithinStage(stageX, stageY)
  }
  ...
  updateQuality(dprScale = 1.0, antialias?: boolean): void {
    const currentQuality = this.device.getRenderQuality()
    const newQuality = {
      ...currentQuality,
  ...
  private startQualityMonitoring(): void {
    const monitor = () => {
      ...
      if (now - this.lastQualityCheck > 2000) {
        ...
        const adjustment = this.performance.getQualityAdjustment(this.device.getRenderQuality())
  ...
  getStats() {
    return {
      ...
      device: {
        tier: this.device.getDeviceTier().tier,
        quality: this.device.getRenderQuality()
      }
    }
  }
