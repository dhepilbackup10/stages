...
export class StagesEngineEvents {
   ...
   */
  processEvent(type: string, stageX: number, stageY: number, originalEvent: PointerEvent | MouseEvent, objectId?: string): void {
    // Create stage event
   ...
   */
  setupEventHandlers(container: HTMLElement, transformCoordinates: (event: PointerEvent | MouseEvent) => { stageX: number; stageY: number } | null, getObjectsAt: (x: number, y: number) => any[]): void {
